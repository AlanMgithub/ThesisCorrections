...
/*
	unsigned long long perfOutMainBegin[7];
	print_counters(perfOutMainBegin);
	for (i = 0; i < 7; i++)
	{
		printf("\nperfOutMainBegin[%d]=%llu", i, perfOutMainBegin[i]);	
	}
*/
...
/*
	...
	for (i = 0; i < 1; i++)
	{
		...
		// PTHREADS CREATE +++++++
		// +++++++++++++++++++++++ 
		pthread_rc = pthread_create(&thread[i],	NULL, linear_scan, (void *)verify);
		if (pthread_rc) 
		{
	...
	// PTHREADS DESTROY ++++++
	// +++++++++++++++++++++++ 
	for(pthread_t = 0; pthread_t < 1; pthread_t++) 
	{
...
*/
	...
	for (i = 0; i < 7; i++){
		if (i==0){
			printf("\n\t\tSTAGE (3)  :  %llu", perfBegin4[0]);
		}
		else {
			printf(":%llu", perfEnd4[i]-perfBegin4[i]);
		}
	}
	printf("\n+++++++++++++++++++++++++++++++++++++++++++++++++++++\n");
	printf("\n");
