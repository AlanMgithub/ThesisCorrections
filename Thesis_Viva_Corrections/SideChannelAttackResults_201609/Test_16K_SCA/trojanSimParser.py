...
def display(dataMatrix, name, count):
	...
	for i in range(1,rows):#range(0,len(dataMatrix)):	
		...
		for j in range(0,len(dataMatrix[i])):
			if dataMatrix[i][j] < (average + average/2):
				badList.append(j)
			runningSum = 0
			if j < len(dataMatrix[i])-1:
				runningSum = runningSum + dataMatrix[i][j] + dataMatrix[i][j+1]
				dataMatrix[i][j] = runningSum
		'''
	...
	for i in range(1,rows):#range(0,len(dataMatrix)):	
		...
		#plt.hist(dataMatrix[i])
		binwidth = 10
		plt.hist(dataMatrix[i], bins=range(min(dataMatrix[i]), max(dataMatrix[i]) + binwidth, binwidth),alpha=0.5,label=str(name))
		plt.legend(loc='upper left')
		plt.yscale("symlog")
		plt.grid('on')	
	#'''
