...
def display(dataMatrix, name, count):
	...
	for i in range(0,rows):#range(0,len(dataMatrix)):	
		'''
		# Array average
		badList = []
...
def extractData(name, rows, columns, count):
	...
	for line in fd:
		...
		if pattern == 'K' or pattern == 'Core1 - CP0 CountReg':
			found = 0
		elif pattern == 'V' or pattern == 'CP0 CountReg' or pattern == 'Core0 - CP0 CountReg':
			found = 1
		elif pattern == 'T':
			found = 2
		elif pattern == 'M':
