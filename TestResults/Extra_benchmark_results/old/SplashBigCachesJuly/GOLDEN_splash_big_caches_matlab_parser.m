...
%% SCALIING FACTOR
currDir = [1365, 3400, 3085, 3240, 530, 220, 4600, 6750, 114, 248, 314];
oldDir =  [1435, 3550, 3400, 3640, 1000, 235, 5170, 7000, 120, 305, 339];
currSelf = [1400, 3450, 3100, 3290, 516, 208, 4480, 6900, 116, 252, 323];
oldSelf = [1460, 3550, 3440, 3650, 625, 240, 4900, 7300, 120, 279, 340];
for i = 1:11
    factDir(i) = currDir(i)/oldDir(i);
    factSelf(i) = currSelf(i)/oldSelf(i);
end
...
k = 1;
for j = 1:11
    data = mean(generalMasterData(k:k+9,:));
    error = std(generalMasterData(k:k+9,:));
    
    for i = 1:7
       data(i) =  data(i)*factDir(j);
       data(i+7) =  data(i+7)*factSelf(j); 
    end
    
    normaliser = data(1);
    for i = 1:14
       data(i) =  data(i)/normaliser;
       error(i) = 0;%error(i)/normaliser;
    end

    data = reshape(data, [7 2]);
    error = reshape(error, [7 2]);

    subplot(6,2,j);
    barweb(data,error, [], [], [], [], [], FaceColor(21:24,:), [], []);
    % Y-Axis
    ylim([0.5 1.2])
    set(gca,'YTick',0.5:0.1:1.2)
    hold on;box on;grid on;
...
%% Process Data
    ...
    hold on;
        ...
        % LU CON
        subplot(5,1,1);
        barweb(normMeanSelfExt(:,1:4), normStdSelfExt(:,1:4), [], [], [], [], [], FaceColor(21:24,:), [], []);
        grid on;box on;hold on;
        baselinePlot = plot(plotDotX,plotDotY,'LineWidth',1.5,'Color','r');
        uistack(baselinePlot,'bottom');
        % Y-Axis
        ...
        % LU NON CON
        subplot(5,1,2);
        barweb(normMeanSelfExt(:,5:8), normStdSelfExt(:,5:8), [], [], [], [], [], FaceColor(21:24,:), [], []);
        grid on;box on;hold on;
        baselinePlot = plot(plotDotX,plotDotY,'LineWidth',1.5,'Color','r');
        uistack(baselinePlot,'bottom');
        % Y-Axis
        ...
        % RADIX
        subplot(5,1,3);
        barweb(normMeanSelfExt(:,9:12), normStdSelfExt(:,9:12), [], [], [], [], [], FaceColor(21:24,:), [], []);
        grid on;box on;hold on;
        baselinePlot = plot(plotDotX,plotDotY,'LineWidth',1.5,'Color','r');
        uistack(baselinePlot,'bottom');
        % Y-Axis
        ...
        % OCEAN CON
        subplot(5,1,4);
        barweb(normMeanSelfExt(:,13:16), normStdSelfExt(:,13:16), [], [], [], [], [], FaceColor(21:24,:), [], []);
        grid on;box on;hold on;
        baselinePlot = plot(plotDotX,plotDotY,'LineWidth',1.5,'Color','r');
        uistack(baselinePlot,'bottom');
        % Y-Axis
        ...
        % OCEAN NON CON
        subplot(5,1,5);
        barweb(normMeanSelfExt(:,17:20), normStdSelfExt(:,17:20), [], [], [], [], [], FaceColor(21:24,:), [], []);
        grid on;box on;hold on;
        baselinePlot = plot(plotDotX,plotDotY,'LineWidth',1.5,'Color','r');
        uistack(baselinePlot,'bottom');
        % Y-Axis
       ...
       %% Plotting data
        ...
        %# export to PDF and open file
        %print -dpdf -r0 splash_extended_selfinv.pdf
    %}
